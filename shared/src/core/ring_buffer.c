...
void ring_buffer_setup(ring_buffer_t* rb, uint8_t* buffer, uint32_t size){
    ...
    rb->read_index = 0;
    rb->write_index = 0;
    rb->mask = size - 1;    // Assuming that the size of the buffer is some power of 2
}
...
bool ring_buffer_read(ring_buffer_t* rb, uint8_t* byte){    // Here the byte represents, the position at which we want to keep the keep the read byte
